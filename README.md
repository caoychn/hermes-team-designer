...
| 复杂任务 | 5-10 人 | Orchestrator + 4-9 个专业角色 |

优先用小团队解决，能 2-3 人就不凑 8-10 人。
...
**协作模式可选**
设计时明确选择协作模式：流水线（A→B→C 串行）、并行汇总（各角色同时工作）、迭代评审（初稿→评审→修改循环）。不同模式对调度协议的写法影响很大。
...
**协作模式**：流水线（developer → reviewer → qa，串行执行）
